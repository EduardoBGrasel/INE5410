...
pthread_mutex_t mutex;  // criando o mutex


void *incrementor(void *arg) {
    int n_loops = *(int *)arg;
    for (int i = 0; i < n_loops; i++) {
        pthread_mutex_lock(&mutex);  // trancando o mutex 
        contador_global += 1;
        pthread_mutex_unlock(&mutex);  // liberando o mutex
    }
...
int main(int argc, char* argv[]) {
    ...
    pthread_mutex_destroy(&mutex);  // destruindo o mutex
